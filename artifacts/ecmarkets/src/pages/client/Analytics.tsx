  ...
  if (active && payload?.length) {
    ...
    return (
      <div style={{
        ...
        <p style={{ color: '#1F77B4', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", fontSize: 13 }}>
...
export function Analytics() {
  ...
  const stats = [
    { label: 'Total Return',   color: '#1F77B4', value: profitPercent !== null ? `+${profitPercent}%` : '—', icon: TrendingUp },
    { label: 'Win Rate',       color: '#1F77B4', value: winRate !== null ? `${winRate}%` : '—', icon: Target },
    { label: 'Net Profit',     color: '#1F77B4', value: totalProfit !== 0 ? `${totalProfit >= 0 ? '+' : ''}₹${Math.abs(totalProfit).toLocaleString('en-IN')}` : '₹0', icon: Zap },
    { label: 'Total Trades',   color: '#4B7CF3', value: String(totalTrades), icon: Activity },
    { label: 'Profit Factor',  color: '#1F77B4', value: String(profitFactor), icon: Award },
    { label: 'Max Drawdown',   color: '#DC2626', value: '—', icon: TrendingDown },
  ];
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#111827] mb-1">Performance Analytics</h1>
        ...
        {stats.map((s, i) => (
          <div key={i} className="card-stealth p-4">
            ...
            <p className="font-terminal text-lg font-bold tabular-nums"
              style={{
                color: s.label === 'Max Drawdown' ? '#DC2626'
                  : s.label === 'Total Return' || s.label === 'Net Profit' || s.label === 'Win Rate' || s.label === 'Profit Factor' ? '#1F77B4'
                  : '#111827'
              }}>
              {s.value}
            </p>
              ...
              <AreaChart data={equityData} margin={{ left: 0, right: 4 }}>
                <defs>
                  <linearGradient id="equityFillA" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%"  stopColor="#1F77B4" stopOpacity={0.22} />
                ...
                <Area type="monotone" dataKey="value" stroke="#1F77B4" strokeWidth={2}
                  ...
                  activeDot={{ r: 4, fill: '#1F77B4', stroke: 'rgba(10,14,28,0.9)', strokeWidth: 2 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        <div className="card-stealth p-5">
          <h3 className="text-base font-bold text-[#111827] mb-1">Win / Loss Breakdown</h3>
          ...
          {totalTrades > 0 ? (
            <div className="flex items-center gap-6">
              <div className="relative shrink-0">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <p className="font-terminal text-lg font-bold text-[#1F77B4]">{winRate ?? '—'}%</p>
              ...
              <div className="flex-1 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#1F77B4] shrink-0" />
                    <span className="text-xs text-[#4B5563] font-semibold">Winning Trades</span>
                  </div>
                  <span className="font-terminal font-bold text-[#1F77B4] text-sm">{winningTrades}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="font-terminal font-bold text-[#DC2626] text-sm">{losingTrades}</span>
                </div>
                <div className="h-px bg-white/[0.05]" />
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#4B5563] font-semibold">Avg Win</span>
                  <span className="font-terminal font-bold text-[#1F77B4] text-xs">+₹{avgWin.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#4B5563] font-semibold">Avg Loss</span>
                  <span className="font-terminal font-bold text-[#DC2626] text-xs">-₹{avgLoss.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="h-[140px] flex items-center justify-center">
              <div className="text-center">
                ...
                <Link href="/dashboard/binary" className="text-xs text-[#1F77B4] font-semibold mt-2 inline-block hover:text-[#60a5fa] transition-colors">
        ...
        {/* Monthly Returns */}
        <div className="card-stealth p-5">
          <h3 className="text-base font-bold text-[#111827] mb-1">Monthly P&L</h3>
                  ...
                  <BarChart data={monthlyReturns} barCategoryGap="30%">
                    ...
                    <Bar dataKey="return" radius={[4, 4, 0, 0]}>
                      {monthlyReturns.map((entry, i) => (
                        <Cell key={i} fill={entry.return >= 0 ? '#1F77B4' : '#DC2626'} fillOpacity={0.8} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              <div className="flex flex-wrap gap-5 mt-4 pt-4 border-t border-white/[0.05]">
                {bestMonth && (
                  <div>
                    ...
                    <p className="font-terminal text-[#1F77B4] font-bold text-sm">
                ...
                <div>
                  ...
                  <p className="font-terminal text-[#111827] font-bold text-sm">{positiveMonths} / {monthlyReturns.length}</p>
                </div>
                {worstMonth && (
                  <div>
                    <p className="text-[10px] text-[#4B5563] font-bold uppercase tracking-wider mb-1">Worst Month</p>
                    <p className="font-terminal text-[#DC2626] font-bold text-sm">
      ...
      <div className="card-stealth p-4 md:p-6 pb-8">
        <div className="mb-5">
          <h3 className="text-base font-bold text-[#111827]">Performance by Instrument</h3>
        ...
        {instrumentList.length > 0 ? (
            ...
            <div className="md:hidden space-y-2">
              {instrumentList.map((inst) => (
                <div key={inst.name} className="p-3.5 rounded-xl flex items-center justify-between"
                  ...
                  <div>
                    <p className="font-bold text-[#111827] text-sm">{inst.name}</p>
                    <p className="text-[11px] text-[#4B5563]">{inst.trades} trades · {inst.winRate}% win</p>
                  </div>
                  <p className={`font-terminal font-bold text-sm tabular-nums ${inst.profit >= 0 ? 'text-[#1F77B4]' : 'text-[#DC2626]'}`}>
                    ...
                    <tr key={inst.name} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                      <td className="py-3.5 pr-6">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="font-bold text-[#111827]">{inst.name}</span>
                        </div>
                      </td>
                      <td className="py-3.5 pr-6 font-terminal text-[#111827] font-bold">{inst.trades}</td>
                      <td className="py-3.5 pr-6">
                        <div className="flex items-center gap-2">
                          <div className="w-16 bg-white/[0.05] rounded-full h-1.5">
                            <div className="h-full bg-[#1F77B4] rounded-full" style={{ width: `${inst.winRate}%` }} />
                          </div>
                          <span className="font-terminal font-bold text-[#1F77B4] text-xs">{inst.winRate}%</span>
                        </div>
                      </td>
                      <td className="py-3.5 pr-6">
                        <div className="flex items-center gap-1.5">
                          <ArrowUpRight className="w-3.5 h-3.5 text-[#1F77B4]" />
                          <span className="font-terminal font-bold text-[#1F77B4]">{inst.wins}</span>
                        </div>
                      </td>
                      <td className="py-3.5 pr-6">
                        <div className="flex items-center gap-1.5">
                          <ArrowDownRight className="w-3.5 h-3.5 text-[#DC2626]" />
                          <span className="font-terminal font-bold text-[#DC2626]">{inst.losses}</span>
                        </div>
                      </td>
                      <td className="py-3.5">
                        <span className={`font-terminal font-bold tabular-nums ${inst.profit >= 0 ? 'text-[#1F77B4]' : 'text-[#DC2626]'}`}>
                          {inst.profit >= 0 ? '+' : ''}₹{Math.abs(inst.profit).toLocaleString('en-IN')}
                        </span>
                      </td>
                    </tr>
          ...
          <div className="text-center py-12">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
              style={{ background: 'rgba(31,119,180,0.08)', border: '1px solid rgba(31,119,180,0.15)' }}>
              <BarChart3 className="w-7 h-7 text-[#1F77B4]/40" />
            </div>
            ...
            <Link href="/dashboard/binary"
              ...
              style={{ background: 'linear-gradient(135deg, #1F77B4 0%, #155D8B 100%)', color: '#000' }}>
              Open Binary Trading
            </Link>
          </div>
        )}
