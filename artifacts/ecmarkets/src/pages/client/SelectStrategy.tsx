...
const RISK_COLORS: Record<string, string> = { low: '#00C274', medium: '#F59E0B', high: '#CF304A' };
const RISK_BG: Record<string, string>    = { low: '#00C27418', medium: '#F59E0B18', high: '#CF304A18' };
...
export function SelectStrategy() {
  ...
  return (
    <DashboardLayout>
      {/* Header */}
      <div className="mb-7">
        <h1 className="text-2xl font-bold text-[#F8FAFC] mb-1">Auto Trading Strategies</h1>
      ...
      {currentStrategyName ? (
        <div className="mb-6 p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4"
          style={{ background: 'rgba(0,194,116,0.06)', border: '1px solid rgba(0,194,116,0.25)', boxShadow: '0 0 24px rgba(0,194,116,0.06)' }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
              style={{ background: 'rgba(0,194,116,0.15)', border: '1px solid rgba(0,194,116,0.3)' }}>
              <Activity className="w-5 h-5 text-[#00C274]" />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <p className="text-xs font-semibold text-[#00C274] uppercase tracking-wider">Currently Active</p>
                <span className="w-1.5 h-1.5 rounded-full bg-[#00C274] animate-pulse" />
              </div>
              <p className="text-lg font-bold text-[#F8FAFC]">{currentStrategyName}</p>
              <p className="text-xs text-[#4B5563] mt-0.5">
                <span className="text-[#00C274] font-bold">+{getDailyRate(currentStrategyName)}% /day</span>
          ...
          <button
            ...
            style={{ background: 'rgba(207,48,74,0.12)', border: '1px solid rgba(207,48,74,0.3)', color: '#CF304A' }}
          >
        ...
        <div className="mb-6 p-4 rounded-2xl flex items-center gap-3 border border-dashed border-[#2B3139]">
          ...
          <div>
            <p className="font-semibold text-[#F8FAFC] text-sm">No Active Strategy</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <div className="flex gap-2">
          {[
            { key: 'all', label: 'All', color: '#F8FAFC' },
            { key: 'low', label: 'Low Risk', color: '#00C274' },
          ...
          {filtered.map((s: any) => {
            ...
            return (
              <div key={s.id}
                ...
                style={isActive ? { borderColor: 'rgba(0,194,116,0.5)', boxShadow: '0 0 24px rgba(0,194,116,0.08)' } : undefined}>

                {/* Badges */}
                {isActive && (
                  <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[10px] font-bold"
                    style={{ background: 'rgba(0,194,116,0.15)', border: '1px solid rgba(0,194,116,0.35)', color: '#00C274' }}>
                    <CheckCircle className="w-3 h-3" /> ACTIVE
                  </div>
                )}
                {isRazr && !isActive && (
                  <div className="absolute top-3 right-3 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wide"
                    style={{ background: '#00C274', color: '#000' }}>FLAGSHIP</div>
                )}
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-[#F8FAFC] truncate text-sm">{s.name}</h3>
                ...
                <div className="rounded-xl p-3 mb-3 flex items-center justify-between"
                  style={{ background: 'rgba(0,194,116,0.06)', border: '1px solid rgba(0,194,116,0.18)' }}>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div className="rounded-xl p-2.5 text-center" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.04)' }}>
                    <p className="text-sm font-bold text-[#00C274]">{s.winRate}%</p>
                    <p className="text-[10px] text-[#4B5563] mt-0.5">Win Rate</p>
                  </div>
                  <div className="rounded-xl p-2.5 text-center" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.04)' }}>
                    <p className="text-sm font-bold text-[#CF304A]">{s.maxDrawdown}%</p>
                ...
                <div className="flex items-center gap-1.5 text-xs text-[#4B5563] mb-4">
                  ...
                  <span>Min: <span className="font-bold text-[#F8FAFC]">₹{Number(s.minCapital).toLocaleString('en-IN')}</span></span>
                </div>

                {/* Action Button */}
                {isActive ? (
                  <button className="w-full py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 cursor-default"
                    style={{ background: 'rgba(0,194,116,0.12)', border: '1px solid rgba(0,194,116,0.3)', color: '#00C274' }}>
