    ...
    <PublicLayout>
      ...
      <section className="animated-bg py-20 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#E5E7EB]/50 border border-[#1F77B4]/30 text-[#1F77B4] px-4 py-1.5 rounded-full inline-flex text-sm font-semibold mb-6">
            <span className="w-2 h-2 rounded-full bg-[#16A34A] animate-pulse mr-2 mt-1"></span>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg text-[#6B7280] max-w-2xl mx-auto mb-8">
      ...
      {/* STATS BAR */}
      <section className="section-surface border-y border-[#E5E7EB] py-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {[
              ...
              <div key={i} className="py-2">
                <p className="text-2xl font-bold text-[#1F77B4]">
                  {s.value === null ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-[#1F77B4]" /> : s.value}
                </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
            <div>
              ...
              <p className="text-[#6B7280] mt-1">{isLoading ? 'Loading...' : `${filteredStrategies.length} of ${allStrategies.length} strategies shown`}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {filters.map((f) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-bold transition-all border ${activeFilter === f ? 'bg-[#1F77B4] text-black border-[#1F77B4]' : 'border-[#E5E7EB] text-[#6B7280] hover:border-[#1F77B4]/50 hover:text-white'}`}
                >
          ...
          {isLoading ? (
            <div className="flex justify-center items-center py-20">
              <Loader2 className="w-10 h-10 animate-spin text-[#1F77B4]" />
            </div>
          ) : filteredStrategies.length === 0 ? (
            <div className="text-center py-20">
              <TrendingUp className="w-16 h-16 text-[#E5E7EB] mx-auto mb-4" />
              <p className="text-[#6B7280] font-medium">No strategies match this filter.</p>
            </div>
          ) : (
            <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredStrategies.map((s: any, i: number) => (
                <motion.div key={s.id || i} variants={fadeUp} className="card-stealth p-6 flex flex-col hover:border-[#1F77B4]/30 transition-all group">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="flex items-center gap-1 text-[10px] font-bold text-[#16A34A] bg-[#16A34A]/10 px-1.5 py-0.5 rounded shrink-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-xs text-[#6B7280]">{s.markets}</span>
                      </div>
                    </div>
                  </div>

                  <p className="text-sm text-[#6B7280] mb-4 leading-relaxed flex-1">{s.description}</p>

                  {/* Live P&L strip */}
                  {liveStats[s.id] && (
                    <div className="flex items-center justify-between bg-[#FFFFFF] border border-[#16A34A]/20 rounded-xl px-3 py-2 mb-3">
                      <span className="text-[10px] font-semibold text-[#6B7280] uppercase tracking-wide">Live P&amp;L Today</span>
                  ...
                  {/* ROI Banner */}
                  <div className={`flex items-center justify-between rounded-xl px-3 py-2 mb-3 border ${isRazrName(s.name) ? 'bg-[#16A34A]/10 border-[#16A34A]/30' : 'bg-[#1F77B4]/10 border-[#1F77B4]/30'}`}>
                    <div className="text-center">
                      <p className={`text-base font-black tabular-nums ${isRazrName(s.name) ? 'text-[#16A34A]' : 'text-[#1F77B4]'}`}>
                        +{liveStats[s.id] ? liveStats[s.id].dailyRate.toFixed(2) : getDailyBase(s.name).toFixed(2)}%
                      </p>
                      <p className="text-[9px] text-[#6B7280] font-semibold uppercase tracking-wide">Daily ROI ↻</p>
                    </div>
                    <div className="w-px h-8 bg-[#E5E7EB]" />
                    <div className="text-center">
                      <p className="text-base font-black text-[#16A34A]">+{(getDailyBase(s.name) * 30).toFixed(0)}%</p>
                      <p className="text-[9px] text-[#6B7280] font-semibold uppercase tracking-wide">Monthly (30d)</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-2 mb-4">
                    <div className="bg-[#FFFFFF] rounded-xl p-3 text-center border border-[#E5E7EB]">
                      <p className="text-sm font-bold text-[#1F77B4] tabular-nums">
                    ...
                    <div className="bg-[#FFFFFF] rounded-xl p-3 text-center border border-[#E5E7EB]">
                      ...
                      <p className="text-[10px] text-[#6B7280] mt-0.5">Min Capital</p>
                    </div>
                  </div>
      ...
      {!isLoading && allStrategies.length > 0 && (
        <section className="py-16 section-surface border-y border-[#E5E7EB]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              <div className="inline-flex items-center gap-2 bg-[#16A34A]/10 border border-[#16A34A]/30 text-[#16A34A] px-4 py-1.5 rounded-full text-xs font-bold mb-4 uppercase tracking-wider">
                <span className="w-2 h-2 rounded-full bg-[#16A34A] animate-pulse" />
                Live Data · Updates Every 2.8s
              </div>
              <h2 className="text-3xl font-bold text-white mb-3">Full Strategy Comparison</h2>
              <p className="text-[#6B7280] max-w-2xl mx-auto">Side-by-side live metrics for all {allStrategies.length} active strategies</p>
              <div className="flex items-center justify-center gap-6 mt-4 text-xs text-[#6B7280]">
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#1F77B4]" />Standard — 4% daily target</span>
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#16A34A]" />RazrMarket — 8% daily target</span>
              </div>
            </div>
            <div className="card-stealth overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[#E5E7EB] border-b border-[#E5E7EB]">
                      {[
                        { label: 'Strategy', sub: '' },
                        { label: 'Risk', sub: '' },
                        ...
                        <th key={h.label} className="px-4 py-3.5 text-left">
                          <span className="text-[#374151] font-bold text-xs uppercase tracking-wider">{h.label}</span>
                          {h.sub && <span className="block text-[#6B7280] text-[9px] font-normal normal-case tracking-normal mt-0.5">{h.sub}</span>}
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#E5E7EB]">
                    {allStrategies.map((s: any, i: number) => {
                      ...
                      return (
                        <tr key={s.id || i} className={`hover:bg-[#E5E7EB]/50 transition-colors ${i % 2 === 0 ? 'bg-[#FFFFFF]/40' : ''}`}>
                          <td className="px-4 py-3.5">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-white text-xs">{s.name}</span>
                              {isRazr && <span className="text-[8px] font-black bg-[#1F77B4] text-black px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0">FLAGSHIP</span>}
                            </div>
                          </td>
                          <td className="px-4 py-3.5">
                          ...
                          <td className="px-4 py-3.5 text-[#374151] text-xs font-medium">
                            ₹{Number(s.minCapital).toLocaleString('en-IN')}
                          </td>
                          <td className="px-4 py-3.5">
                            <span className="text-xs font-bold text-[#1F77B4] tabular-nums">
                              {displayWinRate.toFixed(1)}%
                            </span>
                          </td>
                          <td className="px-4 py-3.5">
                            <span className={`text-xs font-bold tabular-nums ${isRazr ? 'text-[#16A34A]' : 'text-[#1F77B4]'}`}>
                              +{displayDailyRate.toFixed(2)}%
                            </span>
                            <span className="text-[10px] text-[#6B7280] ml-1">/day</span>
                          </td>
                          <td className="px-4 py-3.5">
                            <span className="text-xs font-bold text-[#16A34A] tabular-nums">+{monthly.toFixed(2)}%</span>
                            <span className="text-[10px] text-[#6B7280] ml-1">/mo</span>
                          </td>
                          <td className="px-4 py-3.5">
                            <span className="text-xs font-bold text-[#DC2626]">{s.maxDrawdown}%</span>
                          </td>
                          <td className="px-4 py-3.5 text-[#6B7280] text-xs">{s.markets}</td>
                          <td className="px-4 py-3.5">
      ...
      {/* EDUCATION */}
      <section className="py-20 section-surface border-t border-[#E5E7EB]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-white mb-3">What is Algorithmic Trading?</h2>
            <p className="text-[#6B7280] max-w-2xl mx-auto">Understand the science behind our trading technology.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={i} className="card-stealth p-8 flex gap-5">
                <div className="w-12 h-12 bg-[#FFFFFF] border border-[#E5E7EB] rounded-xl flex items-center justify-center shrink-0">
                  <card.icon className="w-6 h-6 text-[#1F77B4]" />
                </div>
                <div>
      ...
      {/* CTA */}
      <section className="py-16 section-dark text-center border-t border-[#E5E7EB]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-white mb-4">Ready to Activate a Strategy?</h2>
          <p className="text-[#6B7280] mb-8">Open your account in minutes. No trading experience required.</p>
