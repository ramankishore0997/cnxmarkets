  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'radial-gradient(ellipse at 50% 35%, #0F172A, #07091A, #020617)' }}>
      <div className="text-center max-w-sm w-full mx-4">
        {status === 'loading' && (
          <>
            <Loader2 className="w-12 h-12 text-[#1F77B4] animate-spin mx-auto mb-4" />
        ...
        {status === 'success' && (
          <>
            <ShieldCheck className="w-12 h-12 text-[#16A34A] mx-auto mb-4" />
        ...
        {status === 'error' && (
          <>
            ...
            <p className="text-[#DC2626] text-sm mt-2 font-mono">{error}</p>
            <button
              onClick={() => window.location.reload()}
              className="mt-6 px-6 py-2.5 rounded-xl bg-[#1F77B4] text-black font-bold text-sm hover:bg-[#F8D33A] transition-all"
            >
              Retry
            </button>
          </>
